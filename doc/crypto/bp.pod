...
=head1 SYNOPSIS
 ...
 BIGNUM *G1_ELEM_point2bn(const BP_GROUP *group, const G1_POINT *point,
	point_conversion_form_t form, BIGNUM *bn, BN_CTX *ctx);
 G1_ELEM *G1_ELEM_bn2point(const BP_GROUP *group, const BIGNUM *bn,
	G1_ELEM *point, BN_CTX *ctx);
 char *G1_ELEM_point2hex(const BP_GROUP *group, const G1_ELEM *point,
	point_conversion_form_t form, BN_CTX *ctx);
 G1_ELEM *G1_ELEM_hex2point(const BP_GROUP *group, const char *hex,
	G1_ELEM *point, BN_CTX *ctx);
 ...
 int G1_ELEM_is_at_infinity(const BP_GROUP *group, const G1_ELEM *point);
 int G1_ELEM_is_on_curve(const BP_GROUP *group, const G1_ELEM *point, BN_CTX *ctx);
 int G1_ELEM_cmp(const BP_GROUP *group, const G1_ELEM *point, const G1_ELEM *b, BN_CTX *ctx);
 int G1_ELEM_make_affine(const BP_GROUP *group, G1_ELEM *point, BN_CTX *ctx);
 int G1_ELEMs_make_affine(const BP_GROUP *group, size_t num, G1_ELEM *points[], BN_CTX *ctx);
 int G1_ELEM_mul(const BP_GROUP *group, G1_ELEM *r, const BIGNUM *g_scalar,
	const G1_ELEM *point, const BIGNUM *p_scalar, BN_CTX *ctx);
 int G1_ELEMs_mul(const BP_GROUP *group, G1_ELEM *r, const BIGNUM *scalar, size_t num,
	const G1_ELEM *points[], const BIGNUM *scalars[], BN_CTX *ctx);
 ...
 int G2_ELEM_set_compressed_coordinates(const BP_GROUP *group, G2_ELEM *point,
	const BIGNUM *x[2], int y_bit, BN_CTX *ctx);
 ...
 int G2_ELEM_oct2point(const BP_GROUP *group, G2_ELEM *point,
	const unsigned char *buf, size_t len, BN_CTX *ctx);
 BIGNUM *G2_ELEM_point2bn(const BP_GROUP *group, const G2_POINT *point,
	point_conversion_form_t form, BIGNUM *bn, BN_CTX *ctx);
 G2_ELEM *G2_ELEM_bn2point(const BP_GROUP *group, const BIGNUM *bn,
	G2_ELEM *point, BN_CTX *ctx);
 char *G2_ELEM_point2hex(const BP_GROUP *group, const G2_ELEM *point,
	point_conversion_form_t form, BN_CTX *ctx);
 G2_ELEM *G2_ELEM_hex2point(const BP_GROUP *group, const char *hex,
	G2_ELEM *point, BN_CTX *ctx);
 ...
 int GT_ELEM_oct2elem(const BP_GROUP *group, GT_ELEM *point, const unsigned char *buf,
	size_t len, BN_CTX *ctx);
 BIGNUM *GT_ELEM_elem2bn(const BP_GROUP *group, const GT_elem *elem,
	elem_conversion_form_t form, BIGNUM *bn, BN_CTX *ctx);
 GT_ELEM *GT_ELEM_bn2elem(const BP_GROUP *group, const BIGNUM *bn,
	GT_ELEM *elem, BN_CTX *ctx);
 char *GT_ELEM_elem2hex(const BP_GROUP *group, const GT_ELEM *elem,
	elem_conversion_form_t form, BN_CTX *ctx);
 GT_ELEM *GT_ELEM_hex2elem(const BP_GROUP *group, const char *hex,
	GT_ELEM *elem, BN_CTX *ctx);
...
with embedding degree k = 12 of the form:
...
For BN-curves, G_1 is a q-order cyclic subgroup of E and G_2 is a q-order subgroup of E(F_{p^k}) isomorphic to a subgroup
of E'(Fp^2). The group G_T is the set of q-th roots of unity in the finite field Fp^k.
...
to 3 mod 4 and to 1 mod 6, parameterized by negative integers.
